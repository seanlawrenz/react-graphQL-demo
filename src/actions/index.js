...
export const fetchWebhooks = () => async dispatch => {
  dispatch(requestWebhooks());
  const data = await APIRequest(1, 'TDTickets', 1, '');
  dispatch(receivedWebhooks(data));
};

export const fetchIndividualWebhook = uri => async dispatch => {
...
export const createNewWebhook = webhook => async dispatch => {
...
export const editExistingWebhook = (webhook, uri) => async dispatch => {
  dispatch(editWebhook(webhook, uri));
  const data = await APIRequest(1, 'TDTickets', 1, uri, 'PUT', webhook);
  dispatch(receivedIndividualWebhook(data));
};

export const deleteExistingWebhook = (webhook, uri) => async dispatch => {
